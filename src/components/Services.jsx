        ...
        <div className="grid md:grid-cols-2 gap-8">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-faded-bark border border-deep-mocha p-8 rounded-xl hover:border-rust-accent transition-colors duration-300 group"
              >
                <div className="mb-6 inline-block p-4 bg-pitch-darkness rounded-lg group-hover:bg-rust-accent transition-colors duration-300">
                  <Icon className="w-6 h-6 text-rust-accent group-hover:text-pitch-darkness transition-colors duration-300" />
                </div>
                <h3 className="text-2xl font-bold text-cork-dust mb-3 font-halyard">
                  {service.title}
                </h3>
                <p className="text-aged-stone leading-relaxed">
                  {service.description}
                </p>
              </div>
            )
